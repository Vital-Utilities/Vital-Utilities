...
const COLORS = {
    ...
    cached: "#3b82f6", // Blue - Cached Files
            ...
            bottomItems={
                <>
                    <div style={{ display: "flex", flexWrap: "wrap", alignContent: "flex-start", gap: 15, overflowY: "auto" }}>
                        {/* Main usage */}
                        <ItemOne color="#3b82f6" title="In Use" value={`${getReadableBytesString(ramData?.usedMemoryBytes)} (${usedPercent}%)`} />
...
function drawMemoryBar(ctx: CanvasRenderingContext2D, width: number, height: number, data: MemoryData) {
    ...
    // Calculate segment widths (as percentage of bar)
    const segments: { color: string; label: string; bytes: number; width: number }[] = [];
    const usedTotal = data.app + data.wired + data.compressed + data.cached;

    if (data.app > 0) {
        segments.push({ color: COLORS.app, label: "App", bytes: data.app, width: (data.app / data.total) * barWidth });
    }
    if (data.wired > 0) {
        segments.push({ color: COLORS.wired, label: "Wired", bytes: data.wired, width: (data.wired / data.total) * barWidth });
    }
    if (data.compressed > 0) {
        segments.push({ color: COLORS.compressed, label: "Compressed", bytes: data.compressed, width: (data.compressed / data.total) * barWidth });
    }
    if (data.cached > 0) {
        segments.push({ color: COLORS.cached, label: "Cached", bytes: data.cached, width: (data.cached / data.total) * barWidth });
    }

    // Add free space segment
    if (data.free > 0) {
        segments.push({ color: COLORS.free, label: "Free", bytes: data.free, width: (data.free / data.total) * barWidth });
    }
    ...
    // Draw legend below bar
    const legendY = barY + barHeight + 25;
    const legendItems = segments.filter(s => s.width > 5);
    const legendSpacing = Math.min(120, (barWidth - 20) / legendItems.length);
    let legendX = barX + 10;
