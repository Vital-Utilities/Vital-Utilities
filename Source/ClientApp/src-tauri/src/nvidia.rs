...
fn get_gpu_clock_speeds(device: &Device) -> GpuClockSpeeds {
    GpuClockSpeeds {
        memory_clock_mhz: match device.clock_info(nvml_wrapper::enum_wrappers::device::Clock::Memory) {
            Ok(speed) => Some(speed as i32),
            Err(_) => None,
        },
        compute_clock_mhz: match device.clock_info(nvml_wrapper::enum_wrappers::device::Clock::SM) {
            Ok(speed) => Some(speed as i32),
            Err(_) => None,
        },
        graphics_clock_mhz: match device.clock_info(nvml_wrapper::enum_wrappers::device::Clock::Graphics) {
            Ok(speed) => Some(speed as i32),
            Err(_) => None,
        },
        video_clock_mhz: match device.clock_info(nvml_wrapper::enum_wrappers::device::Clock::Video) {
            Ok(speed) => Some(speed as i32),
            Err(_) => None,
        },
    }
    ...
    if let Some(nvml) = nvml {
        ...
        for i in 0..count.unwrap() {
            ...
            if let Ok(device) = device {
                let util: Result<Vec<nvml_wrapper::struct_wrappers::device::ProcessUtilizationSample>, _> = device.process_utilization_stats(None);
                if let Ok(util) = util {
